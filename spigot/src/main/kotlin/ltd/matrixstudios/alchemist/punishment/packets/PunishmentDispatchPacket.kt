...
class PunishmentDispatchPacket(
    ...
    override fun action() {
        val hoverComponent = Component.text(Chat.format("&6&m-------------------------"))
            .appendNewline()
            .append(Component.text(Chat.format("&eTarget: &r" + AlchemistAPI.getRankDisplay(target))))
            .appendNewline()
            .append(Component.text(Chat.format("&eExecutor: &r" + AlchemistAPI.getRankDisplay(executor))))
            .appendNewline()
        ...
        val component = Component.text(Chat.format((if (silent) "&7[Silent]" else "") + " &r" +
                ...
                " &r" +
                AlchemistAPI.getRankDisplay(target) + "&a.")).hoverEvent(HoverEvent.showText(hoverComponent))
