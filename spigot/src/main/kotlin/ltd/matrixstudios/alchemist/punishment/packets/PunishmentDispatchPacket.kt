                ...
                Component.text("Target: ").color(NamedTextColor.YELLOW)
                    .append(
                        Component.text(target?.username ?: "Console").color(
                            Chat.findTextColorFromString(
                                target?.getCurrentRank()?.color ?: AlchemistAPI.CONSOLE_COLOR
                            )
                        )
                    )
                    .appendNewline()
                    ...
                    .append(
                        Component.text(profile?.username ?: "Console").color(
                            Chat.findTextColorFromString(
                                profile?.getCurrentRank()?.color ?: AlchemistAPI.CONSOLE_COLOR
                            )
                        )
                    )
            )
        ...
        val component = Component.text(Chat.format((if (silent) "&7(Silent) " else "") + "&r"))
            .append(
                Component.text(profile?.username ?: "Console")
                    .color(Chat.findTextColorFromString(profile?.getCurrentRank()?.color ?: AlchemistAPI.CONSOLE_COLOR))
            )
            .append(
                Component.text(
                    Chat.format(
            ...
            .append(
                Component.text(target?.username ?: "Console")
                    .color(Chat.findTextColorFromString(target?.getCurrentRank()?.color ?: AlchemistAPI.CONSOLE_COLOR))
