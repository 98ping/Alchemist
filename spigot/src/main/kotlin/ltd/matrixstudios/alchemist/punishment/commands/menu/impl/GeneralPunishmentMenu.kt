            ...
            5 to SkullButton(
                ...
                if (next > limit)
                {
                    GeneralPunishmentMenu(
                        profile,
                        punishmentType,
                        values[0].lambda.invoke(PunishmentService.getFromCache(profile.uuid)).toMutableList(),
                        values[0],
                        player
                    ).updateMenu()
                ...
                GeneralPunishmentMenu(
                    profile,
                    punishmentType,
                    values[next].lambda.invoke(PunishmentService.getFromCache(profile.uuid)).toMutableList(),
                    values[next],
                    player
                ).updateMenu()
            },
