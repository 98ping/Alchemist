    ...
    override fun getHeaderItems(player: Player): MutableMap<Int, Button>
    {
        ...
        buttons[5] = SimpleActionButton(
            Material.EMERALD, mutableListOf(
                Chat.format(" "),
                Chat.format("&7Apply this grant using the"),
                Chat.format("&7scope shown in the summary."),
                Chat.format(" "),
                if (!global && equipped.isEmpty())
                    Chat.format("&cSelect a server or enable global first")
                else
                    Chat.format("&e&lClick &eto finalize this grant"),
                Chat.format(" ")
            ), Chat.format("&a&lFinalize Grant"), 0
        ).setBody { player, _, _ ->
            if (!global && equipped.isEmpty())
            {
