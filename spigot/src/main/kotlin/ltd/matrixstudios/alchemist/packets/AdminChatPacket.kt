...
class AdminChatPacket(val message: String, val server: String, val sender: UUID) : RedisPacket("admin-message")
{
    ...
    override fun action()
    {
        ...
        val msg =
            AlchemistAPI.AC_FORMAT.replace("%server%", server).replace("%profile%", name).replace("%message%", message)
        Bukkit.getOnlinePlayers().filter { it.hasPermission("alchemist.admin") }
