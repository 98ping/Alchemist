...
class GamemodeCommands : BaseCommand()
{
    ...
    fun gmc(player: Player, @Name("target") @Optional target: OnlinePlayer?)
    {
        if (target == null)
        {
            ...
            AsynchronousRedisSender.send(StaffActionAlertPacket("has gone into CREATIVE mode", player.name))
        } else
        {
            ...
            AsynchronousRedisSender.send(StaffActionAlertPacket("has put ${target.player.name} into CREATIVE mode", player.name))
        }
    }
    ...
    fun gms(player: Player, @Name("target") @Optional target: OnlinePlayer?)
    {
        if (target == null)
        {
            ...
            AsynchronousRedisSender.send(StaffActionAlertPacket("has gone into SURVIVAL mode", player.name))
        } else
        {
            ...
            AsynchronousRedisSender.send(StaffActionAlertPacket("has put ${target.player.name} into SURVIVAL mode", player.name))
        }
    }
}