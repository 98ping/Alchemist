...
class FriendsListMenu(val player: Player, val profile: GameProfile, val filter: FriendFilter) : PaginatedMenu(18, player) {
    ...
    class FriendButton(val profile: GameProfile) : Button()
    {
        ...
        override fun getDescription(player: Player): MutableList<String>? {
            ...
            if (profile.isOnline()) {
                ...
                desc.add(Chat.format("&7&oOffline For " + TimeUtil.formatDuration(System.currentTimeMillis().minus(profile.lastSeenAt))))
            }
            desc.add(Chat.format("&7&m-------------------"))
